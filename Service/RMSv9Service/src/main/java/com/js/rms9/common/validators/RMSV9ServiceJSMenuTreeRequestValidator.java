...
public class RMSV9ServiceJSMenuTreeRequestValidator implements Validator {
	...
	public JSMenuTreeView validateRMSUser(RMSUser user) {
		Map<String, String> map = new HashMap<>();
		MapBindingResult err = new MapBindingResult(map, String.class.getName());
		this.validate(user, err);
		if (err.hasErrors()) {
			return new JSMenuTreeView(ApplicationErrorMsgConstants.ERROR_MSG, err.getFieldErrors().get(0).getCode(),
